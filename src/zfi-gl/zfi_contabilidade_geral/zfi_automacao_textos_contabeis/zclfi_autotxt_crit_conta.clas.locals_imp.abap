CLASS lcl_SelConta DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS preencheIdSelecao FOR DETERMINE ON MODIFY
      IMPORTING keys FOR SelConta~preencheIdSelecao.
    METHODS validaPreenchimento FOR VALIDATE ON SAVE
      IMPORTING keys FOR SelConta~validaPreenchimento.
    METHODS buscaRegraConflitante FOR VALIDATE ON SAVE
      IMPORTING keys FOR SelConta~buscaRegraConflitante.

    METHODS busca_proximo_IdSelecao
      IMPORTING iv_idregra       TYPE ztfi_sel_conta-id_regra
      RETURNING VALUE(rv_result) TYPE ztfi_sel_conta-cod_criterio.


ENDCLASS.

CLASS lcl_SelConta IMPLEMENTATION.
  ...
  METHOD preencheIdSelecao.
    ...
    SORT lt_screen_info BY CritSelAccount.
    READ TABLE lt_screen_info TRANSPORTING NO FIELDS
        WITH KEY CritSelAccount = ''
        BINARY SEARCH.
    ...
    LOOP AT lt_screen_info ASSIGNING FIELD-SYMBOL(<fs_screen_info>).
      IF <fs_screen_info>-CritSelAccount IS NOT INITIAL.
    ...
    MODIFY ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
      ENTITY SelConta
        UPDATE FIELDS ( CritSelAccount )
        WITH VALUE #( FOR ls_screen_info IN lt_screen_info (
                            %key    = ls_screen_info-%key
                            CritSelAccount = me->busca_proximo_idselecao( ls_screen_info-IdRegra )
                           ) )
  ...
  METHOD validaPreenchimento.
    ...
    READ ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
     ENTITY SelConta
       FIELDS ( SignAccount
                OptAccount
                LowAccount
                HighAccount ) WITH CORRESPONDING #( keys )
   RESULT DATA(lt_campos) FAILED DATA(ls_erros).
    ...
    LOOP AT lt_campos ASSIGNING FIELD-SYMBOL(<fs_campos>).
      ...
      IF <fs_campos>-SignAccount IS INITIAL.
        ...
        APPEND VALUE #(
          ...
          %element-SignAccount = if_abap_behv=>mk-on
        ) TO reported-selconta.

      ENDIF.

      IF <fs_campos>-OptAccount IS INITIAL.
        ...
        APPEND VALUE #(
          ...
          %element-OptAccount = if_abap_behv=>mk-on
        ) TO reported-selconta.

      ENDIF.

      IF <fs_campos>-LowAccount IS INITIAL.
        ...
        APPEND VALUE #(
          ...
          %element-lowAccount = if_abap_behv=>mk-on
        ) TO reported-selconta.

      ENDIF.

      CASE <fs_campos>-OptAccount.

        WHEN rsmds_c_option-equal.

          IF <fs_campos>-HighAccount IS NOT INITIAL.
            ...
            APPEND VALUE #(
              ...
              %msg        =  new_message(
                ...
                v1 = <fs_campos>-OptAccount
              )
              %element-highAccount = if_abap_behv=>mk-on
            ) TO reported-selconta.
        ...
        WHEN rsmds_c_option-between.
          ...
          IF <fs_campos>-HighAccount IS INITIAL.
          ...
          IF <fs_campos>-HighAccount LT <fs_campos>-LowAccount.
    ...
    READ ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
      ENTITY SelConta
        FIELDS ( IdRegra
                 ...
                 LowAccount
                 HighAccount ) WITH CORRESPONDING #( keys )
    RESULT DATA(lt_criterio_draft) FAILED DATA(ls_erros_conta).

IF lt_criterio_draft IS NOT INITIAL.

    gt_range_conta = VALUE #( FOR ls_sel_conta IN lt_criterio_draft
    ...
    IF sy-subrc EQ 0.
      ...
      reported-selconta = VALUE #( FOR <fs_regra_gravada> IN gt_regras_gravadas
      ...
      failed-selconta = VALUE #( FOR <fs_reported> IN reported-selconta
                                    ( %tky = <fs_reported>-%tky )
                          ).
