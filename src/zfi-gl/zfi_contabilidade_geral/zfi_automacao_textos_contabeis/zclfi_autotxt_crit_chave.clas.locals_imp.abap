CLASS lcl_SelChave DEFINITION INHERITING FROM cl_abap_behavior_handler.
  ...
  PRIVATE SECTION.
    ...
    METHODS preencheIdSelecao FOR DETERMINE ON MODIFY
      IMPORTING keys FOR SelChave~preencheIdSelecao.
    METHODS validaPreenchimento FOR VALIDATE ON SAVE
      IMPORTING keys FOR SelChave~validaPreenchimento.
    METHODS buscaRegraConflitante FOR VALIDATE ON SAVE
      IMPORTING keys FOR SelChave~buscaRegraConflitante.

    METHODS busca_proximo_IdSelecao
      IMPORTING iv_idregra       TYPE ztfi_sel_chave-id_regra
      RETURNING VALUE(rv_result) TYPE ztfi_sel_chave-cod_criterio.


ENDCLASS.

CLASS lcl_SelChave IMPLEMENTATION.
  ...
  METHOD preencheIdSelecao.
    ...
    SORT lt_screen_info BY CritSelPostingKey.
    READ TABLE lt_screen_info TRANSPORTING NO FIELDS
        WITH KEY CritSelPostingKey = ''
        BINARY SEARCH.
    ...
    LOOP AT lt_screen_info ASSIGNING FIELD-SYMBOL(<fs_screen_info>).
      IF <fs_screen_info>-CritSelPostingKey IS NOT INITIAL.
    ...
    MODIFY ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
      ENTITY SelChave
        UPDATE FIELDS ( CritSelPostingKey )
        WITH VALUE #( FOR ls_screen_info IN lt_screen_info (
                            %key    = ls_screen_info-%key
                            CritSelPostingKey = me->busca_proximo_idselecao( ls_screen_info-IdRegra )
                           ) )
  ...
  METHOD validaPreenchimento.
    ...
    READ ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
     ENTITY SelChave
       FIELDS ( SignPostingKey
                OptPostingKey
                LowPostingKey
                HighPostingKey ) WITH CORRESPONDING #( keys )
   RESULT DATA(lt_campos) FAILED DATA(ls_erros).
    ...
    LOOP AT lt_campos ASSIGNING FIELD-SYMBOL(<fs_campos>).
      ...
      IF <fs_campos>-SignPostingKey IS INITIAL.
        ...
        APPEND VALUE #(
          ...
          %element-SignPostingKey = if_abap_behv=>mk-on
        ) TO reported-selchave.

      ENDIF.

      IF <fs_campos>-OptPostingKey IS INITIAL.
        ...
        APPEND VALUE #(
          ...
          %element-OptPostingKey = if_abap_behv=>mk-on
        ) TO reported-selchave.

      ENDIF.

      IF <fs_campos>-LowPostingKey IS INITIAL.
        ...
        APPEND VALUE #(
          ...
          %element-lowPostingKey = if_abap_behv=>mk-on
        ) TO reported-selchave.

      ENDIF.

      CASE <fs_campos>-OptPostingKey.

        WHEN rsmds_c_option-equal.

          IF <fs_campos>-HighPostingKey IS NOT INITIAL.
            ...
            APPEND VALUE #(
              ...
              %msg        =  new_message(
                ...
                v1 = <fs_campos>-OptPostingKey
              )
              %element-highPostingKey = if_abap_behv=>mk-on
            ) TO reported-selchave.
        ...
        WHEN rsmds_c_option-between.
          ...
          IF <fs_campos>-HighPostingKey IS INITIAL.
          ...
          IF <fs_campos>-HighPostingKey LT <fs_campos>-LowPostingKey.
  ...
  METHOD buscaRegraConflitante.
    ...
    READ ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
      ENTITY SelChave
        FIELDS ( IdRegra
                 ...
                 LowPostingKey
                 HighPostingKey ) WITH CORRESPONDING #( keys )
    RESULT DATA(lt_criterio_draft) FAILED DATA(ls_erros_chave).

    IF lt_criterio_draft IS NOT INITIAL.

      gt_range_chave_lancto = VALUE #( FOR ls_sel_chave IN lt_criterio_draft
    ...
    IF sy-subrc EQ 0.
      ...
      reported-selchave = VALUE #( FOR <fs_regra_gravada> IN gt_regras_gravadas
      ...
      failed-selchave = VALUE #( FOR <fs_reported> IN reported-selchave
                                    ( %tky = <fs_reported>-%tky )
                          ).
