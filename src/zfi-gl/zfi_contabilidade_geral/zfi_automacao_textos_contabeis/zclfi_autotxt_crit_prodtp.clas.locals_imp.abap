CLASS lcl_SelTipoProduto DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS preencheIdSelecao FOR DETERMINE ON MODIFY
      IMPORTING keys FOR SelTipoProduto~preencheIdSelecao.
    METHODS validaPreenchimento FOR VALIDATE ON SAVE
      IMPORTING keys FOR SelTipoProduto~validaPreenchimento.
    METHODS buscaRegraConflitante FOR VALIDATE ON SAVE
      IMPORTING keys FOR SelTipoProduto~buscaRegraConflitante.

    METHODS busca_proximo_IdSelecao
      IMPORTING iv_idregra       TYPE ztfi_sel_prodtp-id_regra
      RETURNING VALUE(rv_result) TYPE ztfi_sel_prodtp-cod_criterio.

ENDCLASS.

CLASS lcl_SelTipoProduto IMPLEMENTATION.

  METHOD preencheIdSelecao.

    READ ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
      ENTITY SelTipoProduto
      FIELDS ( IdRegra
               CritSelFIProdType  )
      WITH CORRESPONDING #( keys )
    RESULT DATA(lt_screen_info).

    SORT lt_screen_info BY CritSelFIProdType.
    READ TABLE lt_screen_info TRANSPORTING NO FIELDS
        WITH KEY CritSelFIProdType = ''
        BINARY SEARCH.
    ...
    LOOP AT lt_screen_info ASSIGNING FIELD-SYMBOL(<fs_screen_info>).
      IF <fs_screen_info>-CritSelFIProdType IS NOT INITIAL.
    ...
    MODIFY ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
      ENTITY SelTipoProduto
        UPDATE FIELDS ( CritSelFIProdType )
        WITH VALUE #( FOR ls_screen_info IN lt_screen_info (
                            %key    = ls_screen_info-%key
                            CritSelFIProdType = me->busca_proximo_idselecao( ls_screen_info-IdRegra )
                           ) )
  ...
  METHOD validaPreenchimento.
    ...
    READ ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
     ENTITY SelTipoProduto
       FIELDS ( SignFIProdType
                OptFIProdType
                LowFIProdType
                HighFIProdType ) WITH CORRESPONDING #( keys )
   RESULT DATA(lt_campos) FAILED DATA(ls_erros).
    ...
    LOOP AT lt_campos ASSIGNING FIELD-SYMBOL(<fs_campos>).
      ...
      IF <fs_campos>-SignFIProdType IS INITIAL.
        ...
        APPEND VALUE #(
          ...
          %element-SignFIProdType = if_abap_behv=>mk-on
        ) TO reported-seltipoproduto.

      ENDIF.

      IF <fs_campos>-OptFIProdType IS INITIAL.
        ...
        APPEND VALUE #(
          ...
          %element-OptFIProdType = if_abap_behv=>mk-on
        ) TO reported-seltipoproduto.

      ENDIF.

      IF <fs_campos>-LowFIProdType IS INITIAL.
        ...
        APPEND VALUE #(
          ...
          %element-lowFIProdType = if_abap_behv=>mk-on
        ) TO reported-seltipoproduto.

      ENDIF.

      CASE <fs_campos>-OptFIProdType.

        WHEN rsmds_c_option-equal.

          IF <fs_campos>-HighFIProdType IS NOT INITIAL.
            ...
            APPEND VALUE #(
              ...
              %msg        =  new_message(
                ...
                v1 = <fs_campos>-OptFIProdType
              )
              %element-highFIProdType = if_abap_behv=>mk-on
            ) TO reported-seltipoproduto.
        ...
        WHEN rsmds_c_option-between.
          ...
          IF <fs_campos>-HighFIProdType IS INITIAL.
          ...
          IF <fs_campos>-HighFIProdType LT <fs_campos>-LowFIProdType.
  ...
  METHOD buscaRegraConflitante.
    ...
    READ ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
      ENTITY SelTipoProduto
        FIELDS ( CritSelFIProdType
                 SignFIProdType
                 OptFIProdType
                 LowFIProdType
                 HighFIProdType ) WITH CORRESPONDING #( keys )
    RESULT DATA(lt_criterio_draft) FAILED DATA(ls_erros_tipo_prod).

    IF lt_criterio_draft IS NOT INITIAL.

      gt_range_tipo_prod = VALUE #( FOR ls_sel_tipo_prod IN lt_criterio_draft
    ...
    IF sy-subrc EQ 0.
      ...
      reported-seltipoproduto = VALUE #( FOR <fs_regra_gravada> IN gt_regras_gravadas
      ...
      failed-seltipoproduto = VALUE #( FOR <fs_reported> IN reported-seltipoproduto
                                    ( %tky = <fs_reported>-%tky )
                          ).
