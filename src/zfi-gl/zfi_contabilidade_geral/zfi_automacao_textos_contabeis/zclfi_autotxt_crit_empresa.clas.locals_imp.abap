CLASS lcl_SelEmpresa DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS preencheIdSelecao FOR DETERMINE ON MODIFY
      IMPORTING keys FOR SelEmpresa~preencheIdSelecao.

    METHODS validaPreenchimento FOR VALIDATE ON SAVE
      IMPORTING keys FOR SelEmpresa~validaPreenchimento.
    METHODS buscaRegraConflitante FOR VALIDATE ON SAVE
      IMPORTING keys FOR SelEmpresa~buscaRegraConflitante.

    METHODS busca_proximo_IdSelecao
      IMPORTING iv_idregra       TYPE ztfi_sel_empr-id_regra
      RETURNING VALUE(rv_result) TYPE ztfi_sel_empr-cod_criterio.

ENDCLASS.

CLASS lcl_SelEmpresa IMPLEMENTATION.
  ...
  METHOD preencheIdSelecao.
    ...
    SORT lt_screen_info BY CritSelCompany.
    READ TABLE lt_screen_info TRANSPORTING NO FIELDS
        WITH KEY CritSelCompany = ''
        BINARY SEARCH.
    ...
    LOOP AT lt_screen_info ASSIGNING FIELD-SYMBOL(<fs_screen_info>).
      IF <fs_screen_info>-CritSelCompany IS NOT INITIAL.
    ...
    MODIFY ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
      ENTITY SelEmpresa
        UPDATE FIELDS ( CritSelCompany )
        WITH VALUE #( FOR ls_screen_info IN lt_screen_info (
                            %key    = ls_screen_info-%key
                            CritSelCompany = me->busca_proximo_idselecao( ls_screen_info-IdRegra )
                           ) )
  ...
  METHOD validaPreenchimento.
    ...
    READ ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
     ENTITY SelEmpresa
       FIELDS ( SignCompany
                OptCompany
                LowCompany
                HighCompany ) WITH CORRESPONDING #( keys )
   RESULT DATA(lt_campos) FAILED DATA(ls_erros).
    ...
    LOOP AT lt_campos ASSIGNING FIELD-SYMBOL(<fs_campos>).
      ...
      IF <fs_campos>-SignCompany IS INITIAL.
        ...
        APPEND VALUE #(
          ...
          %element-SignCompany = if_abap_behv=>mk-on
        ) TO reported-selempresa.

      ENDIF.

      IF <fs_campos>-OptCompany IS INITIAL.
        ...
        APPEND VALUE #(
          ...
          %element-OptCompany = if_abap_behv=>mk-on
        ) TO reported-selempresa.

      ENDIF.

      IF <fs_campos>-LowCompany IS INITIAL.
        ...
        APPEND VALUE #(
          ...
          %element-lowCompany = if_abap_behv=>mk-on
        ) TO reported-selempresa.

      ENDIF.

      CASE <fs_campos>-OptCompany.

        WHEN rsmds_c_option-equal.

          IF <fs_campos>-HighCompany IS NOT INITIAL.
            ...
            APPEND VALUE #(
              ...
              %msg        =  new_message(
                ...
                v1 = <fs_campos>-OptCompany
              )
              %element-highCompany = if_abap_behv=>mk-on
            ) TO reported-selempresa.
        ...
        WHEN rsmds_c_option-between.
          ...
          IF <fs_campos>-HighCompany IS INITIAL.
          ...
          IF <fs_campos>-HighCompany LT <fs_campos>-LowCompany.
  ...
  METHOD buscaRegraConflitante.
    ...
    READ ENTITIES OF zi_fi_cfg_auto_textos_regras IN LOCAL MODE
      ENTITY SelEmpresa
        FIELDS ( IdRegra
                 ...
                 LowCompany
                 HighCompany ) WITH CORRESPONDING #( keys )
    RESULT DATA(lt_criterio_draft) FAILED DATA(ls_erros_empresa).

    IF lt_criterio_draft IS NOT INITIAL.

      gt_range_empresa = VALUE #( FOR ls_sel_empresa IN lt_criterio_draft
    ...
    IF sy-subrc EQ 0.
      ...
      reported-selempresa = VALUE #( FOR <fs_regra_gravada> IN gt_regras_gravadas
      ...
      failed-selempresa = VALUE #( FOR <fs_reported> IN reported-selempresa
                                    ( %tky = <fs_reported>-%tky )
                          ).
