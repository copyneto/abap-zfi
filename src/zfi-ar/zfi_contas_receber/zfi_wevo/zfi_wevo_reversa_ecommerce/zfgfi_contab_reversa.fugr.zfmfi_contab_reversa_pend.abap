FUNCTION zfmfi_contab_reversa_pend.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
...
*"     VALUE(IV_DIAS) TYPE  I DEFAULT '5'
...
*****************************************************************
  IF ls_paramtr-tipolancto1(2) EQ lc_accnt_gl.
    ...
    FIND '<' IN ls_paramtr-text1 MATCH OFFSET lv_off.
    IF sy-subrc EQ 0.
      ...
      IF sy-subrc EQ 0.
        ...
        lv_field    = ls_paramtr-text1+lv_ini(lv_tam).
        lv_variavel = ls_paramtr-text1+lv_ini_t(lv_tam_t).

        ASSIGN (lv_field) TO <fs_fld>.

        REPLACE lv_variavel IN ls_paramtr-text1 WITH <fs_fld>.

        ls_accountgl-item_text = ls_paramtr-text1.
...
*****************************************************************
  IF ls_paramtr-tipolancto2(2) EQ lc_accnt_gl.
    ...
    FIND '<' IN ls_paramtr-text2 MATCH OFFSET lv_off.
    IF sy-subrc EQ 0.
      ...
      IF sy-subrc EQ 0.
        ...
        lv_field    = ls_paramtr-text2+lv_ini(lv_tam).
        lv_variavel = ls_paramtr-text2+lv_ini_t(lv_tam_t).

        ASSIGN (lv_field) TO <fs_fld>.

        REPLACE lv_variavel IN ls_paramtr-text2 WITH <fs_fld>.

        ls_accountgl-item_text = ls_paramtr-text2.
  ...
  IF ls_paramtr-tipolancto1 EQ lc_accnt_pa.
    ...
    IF sy-subrc EQ 0.
      ...
      IF sy-subrc EQ 0.
        ...
        ls_accountpayable-item_text = ls_paramtr-text1.
  ...
  IF ls_paramtr-tipolancto2 EQ lc_accnt_pa.
    ...
    IF sy-subrc EQ 0.
      ...
      IF sy-subrc EQ 0.
        ...
        ls_accountpayable-item_text = ls_paramtr-text2.
...
*****************************************************************
  IF ls_paramtr-busca_taxa = lc_bsc_tax.
    ...
    IF lv_lancar EQ abap_true." Fazer lançamento
      ...
      IF lv_tax = 0. " Não Grava LOG e não faz lançaemnto
        CLEAR lv_lancar.
